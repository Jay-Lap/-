...
class KioskApp:
    def __init__(self, root, brain, safe_limits):
        self.root = root
        self.brain = brain
        self.safe_limits = safe_limits 
        self.root.title("영양제 안전 분석 시스템")
        
        # 라즈베리파이 터치 LCD 규격 (800x450)
    ...
    def show_start_page(self):
        ...
        tk.Button(self.content_container, text="키오스크 시작", font=("Helvetica", 16, "bold"), 
                  command=self.show_gender_page, bg="#00FFCC", width=12, height=1).pack(pady=10)
    ...
    def show_brand_page(self, category):
        ...
        for prod in products:
            btn = tk.Button(scroll_frame, text=prod['name'], font=("Helvetica", 11), width=65, pady=6,
    ...
    def show_result_page(self):
        """💡 [핵심 수정] 과다복용(초과)된 성분만 필터링하여 출력하는 결과 스크린"""
        self.clear_frame()
        ...
        any_danger = False
        limit_data = self.safe_limits[self.brain.gender]
        ...
        for nutrient, limit in limit_data.items():
            ...
            # 💡 [핵심직관화 변경] 오직 상한선을 초과(is_over == True)한 성분만 화면 리스트에 추가합니다.
            if is_over:
                any_danger = True
        ...
        # 💡 만약 초과한 성분이 단 하나도 없다면 화면 중앙에 안전 메시지 출력
        if not any_danger:
